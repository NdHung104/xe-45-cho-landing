        ...
        <a href="#top" className="flex items-center gap-2 group">
          ...
          <span
            className={`text-xl font-bold tracking-tight transition-colors ${
              scrolled ? "text-slate-900" : "text-blue-700"
            }`}
          >
            DU LỊCH TÀI HƯNG
          </span>
        ...
        <nav className="hidden md:flex items-center gap-10">
          {MENU.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="group relative py-2 text-sm font-semibold uppercase tracking-wider text-slate-700 hover:text-blue-600 transition"
            >
              {item.label}
        ...
        <div className="flex flex-col p-6 gap-4">
          {MENU.map((item) => (
            <a
              ...
              className="flex items-center justify-between py-4 text-lg font-medium border-b border-slate-100 text-slate-800"
            >
              {item.label}
